...
def contract_from_issue(project: dict[str, Any], state: dict[str, Any], issue: dict[str, Any]) -> dict[str, Any] | None:
    ...
    lowered = text.lower()
    if any(term in lowered for term in RISK_TERMS): return None
    pid = project["id"]
    ...
    if not all((objective, impact, rationale, dod, effect, domain)): return None
    number = int(issue["number"])
    item: dict[str, Any] = {
        "work_item_id": f"{pid}-issue-{number}", "fingerprint": "", "project_id": pid,
        "repository": project["repository"], "issue_number": number,
        "discovery_source": f"github-issue:{number}", "product_objective": objective,
        "user_impact": impact, "rationale": rationale, "acceptance_criteria": criteria,
        "definition_of_done": dod, "target_surfaces": surfaces, "required_tests": tests,
        "expected_product_effect": effect, "collision_domain": domain,
        "exact_input_sha": state["exactSha"], "exact_sha": state["exactSha"],
        "integration_lane": project.get("integrationLane", "integration/razzo"),
        "allowed_surfaces": surfaces, "forbidden_surfaces": [".github/workflows", "infra", "migrations", "secrets", "factory/policy"],
        "dependencies": [], "risk_class": "safe-product", "human_gate": False,
        "evidence_required": evidence or ["non-empty diff", "required tests green", "real product PR"],
        "issue_title": str(issue.get("title") or "")[:240], "issue_body": str(issue.get("body") or "")[:7000],
        "factory_test": project.get("factoryTest", ""), "factory_plan": project.get("factoryPlan", ""),
        "priority_score": 100, "actionability_state": "PENDING",
    }
...
def discover(token: str, limit: int = 3) -> dict[str, Any]:
    ...
    for project in enabled:
        ...
        for issue in issues:
            ...
            if item is None:
                ...
                continue
            state, reasons = validate(item, open_pr_issue_numbers=open_pr_issues, state=persisted)
            item["actionability_state"] = state
            if state == "READY":
                ready.append(item); admitted = True
            else:
                rejected.append({"project_id": project["id"], "issue_number": item["issue_number"], "fingerprint": item["fingerprint"], "state": state, "reasons": reasons})
        if not admitted:
    ...
    used_domains: set[str] = set(); used_fingerprints: set[str] = set()
    for item in ready:
        if item["collision_domain"] in used_domains or item["fingerprint"] in used_fingerprints: continue
        selected.append(item); used_domains.add(item["collision_domain"]); used_fingerprints.add(item["fingerprint"])
...
def main() -> int:
    ...
    if not token: raise SystemExit("PFARMA_SOURCE_TOKEN is required")
