...
def inflight_keys() -> set[tuple[str, str, str]]:
    ...
    return {
        ...
        for item in payload.get("items", []) if item.get("state") == "open"
    }
...
def discover_project_giovanni(root: Path) -> list[dict[str, Any]]:
    ...
    if not (app / "error.tsx").exists():
        items.append(candidate(candidate_id="project-global-error-recovery", product_objective="Add a bounded global application error recovery screen for Project Giovanni.", user_impact="A user receives a clear recovery action instead of an opaque broken page when a route fails.", rationale="The exact Next.js App Router checkout has no app/error.tsx boundary.", acceptance_criteria=["Unhandled route rendering errors show an Italian accessible recovery screen.", "The recovery action invokes reset without external writes."], definition_of_done="A client-side App Router error boundary exists and registry-controlled suites pass.", target_surfaces=["app/error.tsx"], expected_product_effect="Route rendering failures can be recovered without abandoning the session.", collision_domain="product/global-error-recovery", evidence_required=["framework-compatible error boundary", "registry-controlled tests", "non-empty diff"]))
    if not (app / "loading.tsx").exists():
        items.append(candidate(candidate_id="project-global-loading-feedback", product_objective="Add accessible global loading feedback for Project Giovanni route transitions.", user_impact="A user sees immediate screen-reader-compatible feedback while a route is prepared.", rationale="The exact Next.js App Router checkout has no app/loading.tsx fallback.", acceptance_criteria=["Route transitions render concise Italian loading status.", "The status uses a live region and performs no external writes."], definition_of_done="A framework-compatible loading fallback exists and registry-controlled suites pass.", target_surfaces=["app/loading.tsx"], expected_product_effect="Route transitions provide deterministic visual and assistive feedback.", collision_domain="product/global-loading-feedback", evidence_required=["accessible loading fallback", "registry-controlled tests", "non-empty diff"]))
    return items
...
def discover_pfarma_cloud(root: Path) -> list[dict[str, Any]]:
    ...
    if not (root / "api" / "stock_threshold_preview.py").exists():
        items.append(candidate(candidate_id="pfarma-stock-threshold-preview", product_objective="Add a read-only stock threshold preview that classifies urgent, low and healthy inventory without changing stock.", user_impact="The operator can identify products needing attention before creating any replenishment action.", rationale="The exact PFarma checkout has no isolated threshold classification module.", acceptance_criteria=["Classification is deterministic from available, minimum and target quantities.", "The result exposes shortage and suggested replenishment while production_write and purchase_order_write remain false."], definition_of_done="The preview module and focused unit tests exist and the registry-controlled PFarma suites pass.", target_surfaces=["api/stock_threshold_preview.py", "tests/test_stock_threshold_preview.py"], expected_product_effect="PFarma exposes safe inventory attention signals without mutating inventory or suppliers.", collision_domain="inventory/stock-threshold-preview", evidence_required=["focused unit test", "read-only invariants", "non-empty diff"]))
    if not (root / "api" / "interwarehouse_transfer_preview.py").exists():
        items.append(candidate(candidate_id="pfarma-interwarehouse-transfer-preview", product_objective="Add a read-only E-to-A or A-to-E warehouse transfer preview with post-transfer balances and no inventory write.", user_impact="The operator can validate a proposed internal transfer before committing any warehouse movement.", rationale="The exact PFarma checkout has no bounded internal transfer preview.", acceptance_criteria=["Only E and A warehouses are accepted and the source must have sufficient quantity.", "The preview returns post-transfer balances with production_write false."], definition_of_done="The transfer preview module and focused unit tests exist and registry-controlled PFarma suites pass.", target_surfaces=["api/interwarehouse_transfer_preview.py", "tests/test_interwarehouse_transfer_preview.py"], expected_product_effect="PFarma can validate internal warehouse movements without changing real stock.", collision_domain="inventory/interwarehouse-transfer-preview", evidence_required=["focused unit test", "insufficient-stock rejection", "non-empty diff"]))
    return items
...
def discover(project_id: str, root: Path) -> dict[str, Any]:
    detectors: dict[str, Callable[[Path], list[dict[str, Any]]]] = {"project-giovanni": discover_project_giovanni, "pfarma-cloud": discover_pfarma_cloud, "family-cloud": discover_family_cloud}
