...
7. ORBITA AUDIT TEMPO — calcola una durata realistica includendo riscaldamento, serie, recuperi, transizioni e finisher. Non dichiarare mai 30 minuti se il contenuto richiede chiaramente più tempo.
8. ORBITA AUDIT OBIETTIVI — verifica che ogni priorità esplicita dell'utente abbia una conseguenza concreta nel piano. Se chiede spalle e braccia più muscolose, non basta lavoro indiretto: deve esserci volume diretto coerente, salvo vincoli.
9. ORBITA AUDIT CHIAREZZA — nessuna voce operativa deve essere vaga. Un riscaldamento deve essere eseguibile leggendo la scheda, senza dover chiedere spiegazioni dopo.
10. ORBITA AUDIT FINALE — elimina contraddizioni, sovraccarico di una seduta, esercizi ridondanti, diagnosi, misure inventate, body-fat %, promesse di spot reduction e dettagli non realmente visibili.
...
- `timeBudget` deve essere una stima prudente e realistica: warmup + work + transitions + finisher = total, con `total` uguale a `durationMinutes`. Non comprimere artificialmente i recuperi.
