...
const storageStateB64 = process.env.KIMI_STORAGE_STATE_B64 || '';
...
if (!storageStateB64) throw new Error('KIMI_STORAGE_STATE_B64 secret is required');

const storageState = JSON.parse(Buffer.from(storageStateB64, 'base64').toString('utf8'));
...
for (let attempt = 1; attempt <= maxAttempts; attempt++) {
  if (backoff[Math.min(attempt - 1, backoff.length - 1)]) {
...
jobs:
  dispatch:
    ...
    env:
      ...
      KIMI_STORAGE_STATE_B64: ${{ secrets.KIMI_STORAGE_STATE_B64 }}
    steps:
...
Required Actions secret: `KIMI_STORAGE_STATE_B64`.
MD
...
echo "Next: bootstrap one authenticated Kimi browser storage state into GitHub Actions secret KIMI_STORAGE_STATE_B64."
