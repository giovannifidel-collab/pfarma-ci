...
let target=targets.find(t=>t.type==='page'&&/meta\.ai/i.test(t.url||''))||targets.find(t=>t.type==='page');
...
let href=await evalJs(`String(location.href||'')`).catch(()=> '');
if(!/meta\.ai/i.test(href)){
  console.log(`META_NAV_RECOVERY_FROM=${href||'unknown'}`);
  await call('Page.navigate',{url:META_URL});
  const started=Date.now();
  while(Date.now()-started<45000){
    await sleep(500);
    href=await evalJs(`String(location.href||'')`).catch(()=> '');
    const ready=await evalJs(`String(document.readyState||'')`).catch(()=> '');
    if(/meta\.ai/i.test(href)&&ready!=='loading')break;
  }
  console.log(`META_NAV_RECOVERY_TO=${href||'unknown'}`);
}
await sleep(1500);
