...
PROFILE="${GEMINI_LAB_PROFILE_DIR:-$HOME/.hive-agent-lab/gemini-profile}"
PORT="${GEMINI_LAB_CDP_PORT:-9225}"
DISK_CACHE_BYTES="${GEMINI_LAB_DISK_CACHE_BYTES:-134217728}"
MEDIA_CACHE_BYTES="${GEMINI_LAB_MEDIA_CACHE_BYTES:-33554432}"
...
find_browser(){
  if [[ -n "${GEMINI_LAB_BROWSER_BIN:-}" && -x "${GEMINI_LAB_BROWSER_BIN}" ]]; then
    printf '%s\n' "$GEMINI_LAB_BROWSER_BIN"
    return 0
  fi
...
if [[ ! -d node_modules/playwright ]]; then
  echo "GEMINI LAB: installazione dipendenze Node..."
  npm install
fi

BROWSER="$(find_browser || true)"
if [[ -z "$BROWSER" ]]; then
  echo "GEMINI LAB: Chromium non presente; bootstrap Playwright Chromium..."
  npx playwright install chromium
  BROWSER="$(find_browser || true)"
fi
if [[ -z "$BROWSER" ]]; then
  echo "ERROR: Chromium non disponibile."
  exit 1
fi

echo "GEMINI LAB BROWSER=$BROWSER"
...
for _ in $(seq 1 75); do
  if curl -fsS "http://127.0.0.1:${PORT}/json/version" >/dev/null 2>&1; then
    ...
    echo "Accedi a Gemini con il tuo account Google una sola volta; eventuali verifiche umane vanno completate manualmente."
    exit 0
  fi
  sleep 0.4
done
