...
const TEST_ID='HIVE-DUCK-STRESS-0001-FRESH';
...
async function verifySubmitted(token,timeout=12000){const s=Date.now();while(Date.now()-s<timeout){const u=await uiState(),b=await bodyText();if(!u.composerText.includes(nonce)&&count(b,token)>=1)return true;await sleep(300);}return false;}
...
async function send(text,token,label){if(!await waitIdle())throw new Error(`DUCK_NOT_IDLE_BEFORE_${label}`);await quotaCheck();await clearComposer();await call('Input.insertText',{text});await sleep(450);let u=await uiState();if(!u.composerText.includes(nonce))throw new Error(`DUCK_TEXT_NOT_INSERTED_${label}`);let method='ASK_BUTTON';if(!(u.askVisible&&!u.askDisabled&&await clickAsk())){method='ENTER';await pressEnter();}log('SUBMIT_ATTEMPT',label,method);if(!await verifySubmitted(token))throw new Error(`DUCK_PROMPT_NOT_SUBMITTED_${label}`);log('SUBMITTED',label,method,'composer-cleared+prompt-visible');return method;}
...
async function main(){
  ...
  console.log('=== DUCK AI CERTIFICATION ===');
