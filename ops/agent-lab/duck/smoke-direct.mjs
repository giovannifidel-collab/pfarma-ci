...
const expected=`DUCK_SMOKE_OK:${nonce}`;
const prompt=`HIVE Duck.ai smoke test ${nonce}. Rispondi ESATTAMENTE e senza altro testo: ${expected}`;
...
let target=targets.find(t=>t.type==='page'&&/duck\.ai/i.test(t.url||''));
if(!target){target=await json(`${CDP}/json/new?${encodeURIComponent('https://duck.ai/')}`,{method:'PUT'});await sleep(2500);}
...
while(Date.now()-start<45000){
  state=await evalJs(`(()=>{const visible=e=>{const r=e.getBoundingClientRect(),s=getComputedStyle(e);return r.width>0&&r.height>0&&s.display!=='none'&&s.visibility!=='hidden'};const composer=[...document.querySelectorAll('textarea,input,[contenteditable="true"],[role="textbox"]')].filter(visible).find(e=>/ask anything privately/i.test(String(e.getAttribute('placeholder')||e.getAttribute('aria-label')||'')))||[...document.querySelectorAll('textarea')].filter(visible)[0]||null;const ask=[...document.querySelectorAll('button,[role="button"]')].filter(visible).find(e=>/^ask$/i.test(String(e.getAttribute('aria-label')||e.innerText||'').trim()));const body=String(document.body?.innerText||'');const fast=/\bFast\b/.test(body);const model=[...document.querySelectorAll('button,[role="button"]')].filter(visible).map(e=>String(e.innerText||'').trim()).find(t=>/^5\.6\s+Luna$/i.test(t))||null;return {href:String(location.href||''),ready:document.readyState,composer:!!composer,composerTag:composer?.tagName?.toLowerCase()||null,ask:!!ask,fast,model,body};})()`);
  if(state.composer&&state.ask&&state.ready!=='loading')break;
  await sleep(500);
}
if(!state.composer)throw new Error(`DUCK_COMPOSER_NOT_READY href=${state.href||''}`);
if(!state.ask)throw new Error('DUCK_ASK_BUTTON_NOT_READY');
console.log(`DUCK_COMPOSER=${state.composerTag||'unknown'}`);
console.log(`MODE_FAST_VISIBLE=${state.fast?'true':'false'}`);
...
const focused=await evalJs(`(()=>{const visible=e=>{const r=e.getBoundingClientRect(),s=getComputedStyle(e);return r.width>0&&r.height>0&&s.display!=='none'&&s.visibility!=='hidden'};const e=[...document.querySelectorAll('textarea,input,[contenteditable="true"],[role="textbox"]')].filter(visible).find(x=>/ask anything privately/i.test(String(x.getAttribute('placeholder')||x.getAttribute('aria-label')||'')))||[...document.querySelectorAll('textarea')].filter(visible)[0]||null;if(!e)return false;e.focus();return true;})()`);
if(!focused)throw new Error('DUCK_COMPOSER_NOT_FOUND');
...
await sleep(500);
const inserted=await evalJs(`(()=>{const visible=e=>{const r=e.getBoundingClientRect(),s=getComputedStyle(e);return r.width>0&&r.height>0&&s.display!=='none'&&s.visibility!=='hidden'};const e=[...document.querySelectorAll('textarea,input,[contenteditable="true"],[role="textbox"]')].filter(visible).find(x=>/ask anything privately/i.test(String(x.getAttribute('placeholder')||x.getAttribute('aria-label')||'')))||[...document.querySelectorAll('textarea')].filter(visible)[0]||null;return e?String(e.value||e.innerText||e.textContent||''):'';})()`);
if(!inserted.includes(nonce))throw new Error('DUCK_TEXT_NOT_INSERTED');
...
if(!clicked)throw new Error('DUCK_SUBMIT_FAILED');
console.log('DUCK_SMOKE_SENT=true');
console.log(`NONCE=${nonce}`);
console.log('SUBMIT_METHOD=ASK_BUTTON');
...
while(Date.now()-waitStart<120000){
  ...
  if(/daily limit|rate limit|try again later|usage limit|you've reached/.test(low)){
    console.log('DUCK_SMOKE_QUOTA_BLOCK=true');
    console.log(`BODY_TAIL=${JSON.stringify(body.slice(-5000))}`);
    ws.close();process.exit(4);
  }
  await sleep(750);
}
console.log(`DUCK_SMOKE_CAPTURE=${captured?'true':'false'}`);
console.log(`EXPECTED=${expected}`);
console.log(`ACTUAL=${actual}`);
console.log(`DUCK_SMOKE_PASS=${captured&&actual===expected?'true':'false'}`);
