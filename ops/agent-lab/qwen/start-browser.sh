...
PROFILE="${QWEN_LAB_PROFILE_DIR:-$HOME/.hive-agent-lab/qwen-profile}"
PORT="${QWEN_LAB_CDP_PORT:-9228}"
DISK_CACHE_BYTES="${QWEN_LAB_DISK_CACHE_BYTES:-134217728}"
MEDIA_CACHE_BYTES="${QWEN_LAB_MEDIA_CACHE_BYTES:-33554432}"
...
find_browser(){
  if [[ -n "${QWEN_LAB_BROWSER_BIN:-}" && -x "${QWEN_LAB_BROWSER_BIN}" ]]; then
    printf '%s\n' "$QWEN_LAB_BROWSER_BIN"
    return 0
  fi
...
if [[ ! -d node_modules/playwright ]]; then
  echo "QWEN LAB: installazione dipendenze Node..."
  npm install
fi

BROWSER="$(find_browser || true)"
if [[ -z "$BROWSER" ]]; then
  echo "QWEN LAB: Chromium non presente; bootstrap Playwright Chromium..."
...
for _ in $(seq 1 75); do
  if curl -fsS "http://127.0.0.1:${PORT}/json/version" >/dev/null 2>&1; then
    ...
    echo "Accedi a Qwen manualmente una sola volta se richiesto; non inserire credenziali nel terminale."
    exit 0
  fi
  sleep 0.4
done
