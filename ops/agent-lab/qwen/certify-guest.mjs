...
async function uiInfo(){return evalJs(`(()=>{const b=String(document.body?.innerText||'');const model=[...document.querySelectorAll('button,[role="button"]')].map(e=>String(e.innerText||'').trim()).find(t=>/^Qwen/i.test(t))||null;return {model,guest:/\bLog in\b/i.test(b)&&/\bSign up\b/i.test(b),body:b};})()`);}
...
async function main(){
  ...
  const cert={test_id:TEST_ID,nonce,provider:'chat.qwen.ai',transport:'guest-browser-session-direct-cdp',guest_access:true,model_visible:info.model,api_required:false,zero_cost_path:true,started_at:startedAt,completed_at:new Date().toISOString(),expected:{master:MASTER,sums:SUMS,terms:TERMS,checksum:EXPECTED},actual:final,stages:{master:true,shard_a:true,shard_b:true,shard_c:true,final:true},stage_validation:'exact-data-echo',certified:passed};
  const file=path.join(OUT_DIR,`${TEST_ID}-${nonce}.json`);fs.writeFileSync(file,JSON.stringify(cert,null,2));
  console.log('');console.log(`QWEN_CERTIFIED=${passed?'true':'false'}`);console.log(`TEST_ID=${TEST_ID}`);console.log(`NONCE=${nonce}`);console.log(`GUEST_ACCESS=true`);console.log(`MODEL=${info.model||'unknown'}`);console.log(`EXPECTED_MASTER=${MASTER.salt}:${MASTER.coeff.join(',')}`);console.log(`ACTUAL_MASTER=${final.salt}:${final.coeff.join(',')}`);console.log(`EXPECTED_SUMS=${SUMS.A},${SUMS.B},${SUMS.C}`);console.log(`ACTUAL_SUMS=${final.sums.A},${final.sums.B},${final.sums.C}`);console.log(`EXPECTED_TERMS=${TERMS.A},${TERMS.B},${TERMS.C}`);console.log(`ACTUAL_TERMS=${final.terms.A},${final.terms.B},${final.terms.C}`);console.log(`EXPECTED_CHECKSUM=${EXPECTED}`);console.log(`ACTUAL_CHECKSUM=${final.checksum}`);console.log(`CERTIFICATE=${file}`);
