...
const expected=`QWEN_SMOKE_OK:${nonce}`;
...
if(!target){
  target=await json(`${CDP}/json/new?${encodeURIComponent('https://chat.qwen.ai/')}`,{method:'PUT'});
  await sleep(2500);
}
...
while(Date.now()-start<45000){
  state=await evalJs(`(()=>{
    ...
    const ta=[...document.querySelectorAll('textarea')].find(e=>{
      ...
      return String(e.getAttribute('placeholder')||'').toLowerCase().includes('ask qwen') && r.width>0 && r.height>0 && s.display!=='none' && s.visibility!=='hidden';
    });
    return {href:String(location.href||''),ready:document.readyState,textarea:!!ta,body};
  })()`);
  if(state?.textarea&&state.ready!=='loading') break;
  await sleep(500);
}
if(!state?.textarea) throw new Error(`QWEN_COMPOSER_NOT_READY href=${state?.href||''}`);

const esc=s=>s.replace(/[.*+?^${}()|[\]\\]/g,'\\$&');
const tokenRe=new RegExp(esc(expected),'g');
const before=(state.body.match(tokenRe)||[]).length;
const prompt=`HIVE Qwen smoke test ${nonce}. Rispondi ESATTAMENTE e senza altro testo: ${expected}`;

const focused=await evalJs(`(()=>{
  const e=[...document.querySelectorAll('textarea')].find(x=>String(x.getAttribute('placeholder')||'').toLowerCase().includes('ask qwen')&&x.getBoundingClientRect().width>0);
  if(!e) return false;
  e.focus();
  return true;
})()`);
if(!focused) throw new Error('QWEN_COMPOSER_NOT_FOUND');
...
const composer=await evalJs(`(()=>{const e=[...document.querySelectorAll('textarea')].find(x=>String(x.getAttribute('placeholder')||'').toLowerCase().includes('ask qwen')&&x.getBoundingClientRect().width>0);return e?String(e.value||''):''})()`);
if(!composer.includes(expected)) throw new Error(`QWEN_TEXT_NOT_INSERTED len=${composer.length}`);

await call('Input.dispatchKeyEvent',{type:'rawKeyDown',key:'Enter',code:'Enter',windowsVirtualKeyCode:13});
await call('Input.dispatchKeyEvent',{type:'keyUp',key:'Enter',code:'Enter',windowsVirtualKeyCode:13});
console.log('QWEN_SMOKE_SENT=true');
...
while(Date.now()-waitStart<90000){
  ...
  if(count>=before+2){
    console.log('QWEN_SMOKE_CAPTURE=true');
    console.log(`EXPECTED=${expected}`);
    console.log(`ACTUAL=${expected}`);
    console.log('QWEN_SMOKE_PASS=true');
    ws.close();
    process.exit(0);
  }
  const low=body.toLowerCase();
  if(low.includes('log in') && (low.includes('continue with google')||low.includes('continue with github')||low.includes('enter your email'))){
    console.log('QWEN_SMOKE_BLOCKED_BY_LOGIN=true');
...
console.log('QWEN_SMOKE_CAPTURE=false');
