...
function cliAvailable(){
  const r=spawnSync('bash',['-lc','command -v kimi >/dev/null 2>&1'],{stdio:'ignore'});
  return r.status===0;
}

function parseEnvelope(stdout, begin, end){
  const s=String(stdout||'');
  const i=s.lastIndexOf(begin);
  if(i<0)return null;
...
export class KimiHybridAgent{
  constructor(config,opts={}){this.config=config;this.id=config.id;this.browser=new BrowserAgent(config,opts);}
  async health(){
    if(cliAvailable()){
    ...
    if(h.status==='ok')h.metadata={...h.metadata,transport:'browser-cdp-fallback'};
    return h;
  }
  async run(task,options={}){
    ...
    if(cliAvailable()){
      ...
      const text=parseEnvelope(stdout,begin,end);
      if(r.status===0&&text!==null){
        return {status:'ok',text,metadata:{agent_id:this.id,provider:this.config.product,transport:'kimi-cli',zero_cost_path:true,latency_ms:Date.now()-started,nonce}};
      }
