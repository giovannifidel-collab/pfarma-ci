...
export class KimiBrowserAgent extends KeyboardBrowserAgent {
    ...
    return this.eval(`(()=>{
      ...
      for(let depth=0;depth<7&&node;depth++,node=node.parentElement){
        for(const b of node.querySelectorAll?.('button,[role="button"],input[type="submit"]')||[]){
          ...
          let score=0;
          if(/^send$/i.test(label)||/send message|submit|发送|發送|arrow.*up/i.test(label))score+=160;
          if(String(b.getAttribute('type')||'').toLowerCase()==='submit')score+=120;
          if(b.querySelector('svg'))score+=15;
          if(r.left>=cr.left+cr.width*0.55)score+=35;
          if(Math.abs((r.top+r.height/2)-(cr.top+cr.height/2))<Math.max(60,cr.height))score+=20;
          score+=Math.max(0,24-depth*3);
          if(/attach|upload|file|voice|microphone|mic|image|tool|model/i.test(label))score-=120;
          candidates.push({b,score,label,depth,left:r.left,top:r.top});
        }
      }
      candidates.sort((a,b)=>b.score-a.score);
      const hit=candidates[0];
      if(!hit||hit.score<0)return {clicked:false,reason:'no-safe-button'};
      hit.b.click();
  ...
  async submitPrompt(prompt,requestMarker){
    ...
    if(ready&&await this.clickSubmit()){
      ...
      if(await this.verifySubmitted(requestMarker,7000))return 'named-button';
    }
    ...
    if(candidate?.clicked){
      ...
      if(await this.verifySubmitted(requestMarker,8000))return 'candidate-button';
    }

    if(await this.submitNearestForm()){
      attempts.push('nearest-form');
      if(await this.verifySubmitted(requestMarker,8000))return 'nearest-form';
    }

    if(await this.pressEnter()){
      attempts.push('cdp-enter');
      if(await this.verifySubmitted(requestMarker,8000))return 'cdp-enter';
    }

    if(await this.dispatchDomEnter()){
      attempts.push('dom-enter');
      if(await this.verifySubmitted(requestMarker,8000))return 'dom-enter';
    }
