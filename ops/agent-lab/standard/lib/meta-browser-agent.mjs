...
export class MetaBrowserAgent extends MarkerBrowserAgent {
  ...
  async clickMetaStop(){
    const p=await this.eval(`(()=>{const visible=e=>{if(!e)return false;const r=e.getBoundingClientRect(),s=getComputedStyle(e);return r.width>0&&r.height>0&&s.display!=='none'&&s.visibility!=='hidden'};const controls=[...document.querySelectorAll('button,[role="button"]')].filter(visible);const e=[...document.querySelectorAll('[data-testid="composer-stop-button"]')].find(visible)||controls.find(x=>/^(stop|stop generating|stop responding|cancel response)$/i.test(String(x.getAttribute('aria-label')||x.innerText||'').trim()));if(!e||!visible(e)||e.disabled||e.getAttribute('aria-disabled')==='true')return null;const r=e.getBoundingClientRect();return {x:r.left+r.width/2,y:r.top+r.height/2};})()`);
    if(!p)return false;
  ...
  async clickMetaSend(){
    return this.eval(`(()=>{const visible=e=>{if(!e)return false;const r=e.getBoundingClientRect(),s=getComputedStyle(e);return r.width>0&&r.height>0&&s.display!=='none'&&s.visibility!=='hidden'};const e=[...document.querySelectorAll('[data-testid="composer-send-button"]')].find(visible)||[...document.querySelectorAll('button,[role="button"]')].filter(visible).find(x=>/^(send|send message|submit)$/i.test(String(x.getAttribute('aria-label')||x.innerText||'').trim()));if(!e||!visible(e)||e.disabled||e.getAttribute('aria-disabled')==='true')return false;e.click();return true;})()`);
  }
