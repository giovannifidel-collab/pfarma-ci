...
export class BrowserAgent {
  ...
  async freshChat() {
    ...
    if (clicked) { await sleep(1000); await this.waitComposer(30000); }
    return clicked;
  }
  ...
  async health() {
    ...
    try {
      await this.connect();
      const s = await this.waitComposer(30000);
      const status = s.blockedBy ? 'blocked' : 'ok';
      ...
      return this.lastHealth = { status:'error', text:e.message, metadata:{ agent_id:this.id, port:this.port, latency_ms:now()-started } };
    }
  }
