...
if(!target){target=await json(`${CDP}/json/new?${encodeURIComponent('https://chat.mistral.ai/')}`,{method:'PUT'});await sleep(2500);}
...
const info=await evalJs(`(()=>{
  ...
  const body=String(document.body?.innerText||'');const low=body.toLowerCase();
  ...
  const login=low.includes('log in')||low.includes('sign in')||low.includes('continue with google')||low.includes('continue with microsoft')||low.includes('continue with apple');
  const chat=controls.some(x=>x.tag==='textarea'||x.contenteditable==='true')&&(low.includes('mistral')||low.includes('vibe')||low.includes('chat'));
