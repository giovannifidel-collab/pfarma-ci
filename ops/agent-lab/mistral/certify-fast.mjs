...
const OUT_DIR=process.env.MISTRAL_LAB_CERT_DIR||path.resolve('certifications');
const TEST_ID='HIVE-MISTRAL-STRESS-0001';
...
async function composerReady(){
  return evalJs(`(()=>{const visible=e=>{const r=e.getBoundingClientRect(),s=getComputedStyle(e);return r.width>0&&r.height>0&&s.display!=='none'&&s.visibility!=='hidden'};const all=[...document.querySelectorAll('[contenteditable="true"],textarea')].filter(visible);return all.some(e=>e.getAttribute('contenteditable')==='true')||all.length>0})()`);
}

async function uiInfo(){
  return evalJs(`(()=>{const visible=e=>{const r=e.getBoundingClientRect(),s=getComputedStyle(e);return r.width>0&&r.height>0&&s.display!=='none'&&s.visibility!=='hidden'};const body=String(document.body?.innerText||'');const fast=[...document.querySelectorAll('button,[role="button"]')].filter(visible).find(e=>String(e.innerText||'').trim()==='Fast'||String(e.getAttribute('aria-label')||'').trim()==='Fast');const authenticated=!(/\bSign in\b/i.test(body)&&/\bSign up\b/i.test(body)&&!body.includes('Default Workspace'));return {href:String(location.href||''),fastVisible:!!fast,authenticated,mode:fast?'Fast':null};})()`);
}
...
async function send(text){
  if(!await focusComposer())throw new Error('MISTRAL_COMPOSER_NOT_FOUND');
  ...
  await sleep(400);
  const value=await composerText();
  if(!value||value.length<Math.min(20,text.length))throw new Error(`MISTRAL_TEXT_NOT_INSERTED len=${value.length}`);
...
async function waitForExact(expected,before,timeout=150000){
  ...
  while(Date.now()-started<timeout){
    ...
    if(literalCount(b,expected)>=before+2){await sleep(700);return 'body-occurrence-exact';}
    const low=b.toLowerCase();
    if(low.includes('sign in')&&low.includes('sign up')&&!low.includes('default workspace'))throw new Error('MISTRAL_LOGIN_REQUIRED_DURING_CERT');
    await sleep(900);
  }
  throw new Error(`TIMEOUT_WAITING_FOR_EXACT_MISTRAL_RESPONSE_${expected}`);
}
...
async function waitFinal(timeout=210000){const started=Date.now();const re=new RegExp(`MISTRAL_CERT_RESULT:${nonce}:(\\d+):(\\d+):(\\d+):(\\d+):(\\d+):(\\d+):(\\d+):(\\d+):(\\d+):(\\d+):(\\d+)`,'g');while(Date.now()-started<timeout){const b=await bodyText();const matches=[...b.matchAll(re)];if(matches.length)return parseFinal(matches[matches.length-1]);await sleep(1000);}throw new Error(`TIMEOUT_WAITING_FOR_MISTRAL_CERT_RESULT_${nonce}`);}

async function main(){
  fs.mkdirSync(OUT_DIR,{recursive:true});
  const startedAt=new Date().toISOString();
  const start=Date.now();while(Date.now()-start<30000){if(await composerReady())break;await sleep(500);}if(!await composerReady())throw new Error('MISTRAL_COMPOSER_NOT_READY');
  const info=await uiInfo();
  if(!info.authenticated)throw new Error('MISTRAL_NOT_AUTHENTICATED');
  if(!info.fastVisible)throw new Error('MISTRAL_FAST_MODE_NOT_VISIBLE');
  log(`START ${TEST_ID} nonce=${nonce} expected=${EXPECTED} sums=${SUMS.A},${SUMS.B},${SUMS.C} terms=${TERMS.A},${TERMS.B},${TERMS.C}`);
  log(`MODE ${info.mode} authenticated=${info.authenticated}`);
...
main().catch(err=>{console.error('MISTRAL_CERTIFIED=false');console.error(`ERROR=${err.message}`);try{ws.close();}catch{}process.exit(1);});
