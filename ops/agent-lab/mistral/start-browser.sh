...
PROFILE="${MISTRAL_LAB_PROFILE_DIR:-$HOME/.hive-agent-lab/mistral-profile}"
PORT="${MISTRAL_LAB_CDP_PORT:-9229}"
DISK_CACHE_BYTES="${MISTRAL_LAB_DISK_CACHE_BYTES:-134217728}"
MEDIA_CACHE_BYTES="${MISTRAL_LAB_MEDIA_CACHE_BYTES:-33554432}"
...
find_browser(){
  if [[ -n "${MISTRAL_LAB_BROWSER_BIN:-}" && -x "${MISTRAL_LAB_BROWSER_BIN}" ]]; then printf '%s\n' "$MISTRAL_LAB_BROWSER_BIN"; return 0; fi
...
if curl -fsS "http://127.0.0.1:${PORT}/json/version" >/dev/null 2>&1; then
  echo "MISTRAL LAB BROWSER READY"
...
if [[ ! -d node_modules/playwright ]]; then
  echo "MISTRAL LAB: installazione dipendenze Node..."
  npm install
fi

BROWSER="$(find_browser || true)"
if [[ -z "$BROWSER" ]]; then
  echo "MISTRAL LAB: Chromium non presente; bootstrap Playwright Chromium..."
...
if [[ ! -S "/tmp/.X11-unix/X1" ]]; then
  [[ -f "$SHARED_HOST" ]] || { echo "ERROR: shared Agent Lab browser host missing: $SHARED_HOST"; exit 3; }
  bash "$SHARED_HOST"
fi
export DISPLAY=":1"
...
for _ in $(seq 1 75); do
  if curl -fsS "http://127.0.0.1:${PORT}/json/version" >/dev/null 2>&1; then
    ...
    echo "Accedi a Mistral Vibe manualmente una sola volta se richiesto; non inserire credenziali nel terminale."
    exit 0
  fi
  sleep 0.4
done
