...
const TEST='HIVE-GROK-STRESS-0006';
...
async function visible(page,sels){for(const s of sels){const a=page.locator(s),n=await a.count().catch(()=>0);for(let i=n-1;i>=0;i--){const x=a.nth(i);if(await x.isVisible().catch(()=>false))return x;}}return null;}
...
async function send(page,text){
 ...
 await c.click(); const tag=await c.evaluate(e=>e.tagName.toLowerCase()).catch(()=> 'div');
 if(['textarea','input'].includes(tag))await c.fill(text); else await c.fill(text).catch(async()=>c.evaluate((e,t)=>{e.focus();e.textContent=t;e.dispatchEvent(new InputEvent('input',{bubbles:true,composed:true,inputType:'insertText',data:t}));},text));
 ...
 const st=Date.now();while(Date.now()-st<8000){if((await textOf(c).catch(()=> '')).trim().length<5)return;await sleep(250);}throw new Error('GROK_PROMPT_NOT_SUBMITTED');
}

async function freshPage(context){
 for(const p of context.pages())if(p.url().includes('grok.com'))await p.close().catch(()=>{});
 const p=await context.newPage(); await p.goto('https://grok.com/',{waitUntil:'domcontentloaded',timeout:60000});
 const st=Date.now();while(Date.now()-st<30000){if(await composer(p).catch(()=>null))return p;await sleep(1000);}throw new Error(`GROK_FRESH_CHAT_NOT_READY url=${p.url()}`);
}

async function waitToken(page,token,timeout=90000){const st=Date.now();while(Date.now()-st<timeout){if((await body(page)).includes(token))return;await sleep(700);}throw new Error(`TIMEOUT_${token}`);}
async function echoStage(page,label,prompt,echo){log('SEND',label);await send(page,prompt);await waitToken(page,echo);log('PASS',label);}
...
async function askNumber(page,nonce,label,instruction,recheck=false){
 ...
 while(Date.now()-st<90000){const m=re.exec(await body(page));if(m){const v=Number(m[1]);log('RESULT',recheck?`RECHECK_${label}`:label,v);return v;}await sleep(700);}throw new Error(`TIMEOUT_${label}`);
}
...
async function checkedNumber(page,nonce,label,instruction,want){const first=await askNumber(page,nonce,label,instruction,false);if(first===want)return {first,final:first,rechecked:false,ok:true};const second=await askNumber(page,nonce,label,instruction,true);return {first,final:second,rechecked:true,ok:second===want};}

async function trial(context,index,d){
 const nonce=crypto.randomBytes(4).toString('hex').toUpperCase(); const exp=expected(d); const page=await freshPage(context);
 ...
 for(const k of ['A','B','C']){const e=`ACK_${k}:${nonce}:${d.shards[k].join(',')}`;await echoStage(page,`SHARD_${k}`,[`${TEST} / ${nonce}`,`SHARD_${k}=${d.shards[k].join(',')}`,`Memorizza e rispondi ESATTAMENTE: ${e}`].join('\n'),e);}
 ...
 await page.close().catch(()=>{}); return {index,nonce,expected:exp,results,raw_pass:raw,verified_pass:verified};
}

async function main(){
 fs.mkdirSync(OUT,{recursive:true}); const browser=await chromium.connectOverCDP(CDP); const context=browser.contexts()[0]||await browser.newContext(); const trials=[];
 ...
 const file=path.join(OUT,`${TEST}-${Date.now()}.json`);fs.writeFileSync(file,JSON.stringify({test_id:TEST,trials,raw_pass:raw,verified_pass:verified,certification_mode:mode,certified},null,2));
 console.log(`GROK_CERTIFIED=${certified}`);console.log(`TEST_ID=${TEST}`);console.log(`TRIALS=${trials.length}`);console.log(`RAW_PASS=${raw}`);console.log(`VERIFIED_PASS=${verified}`);console.log(`CERTIFICATION_MODE=${mode}`);console.log(`CERTIFICATE=${file}`);
 if(!certified)process.exitCode=2; await browser.close().catch(()=>{});
}
