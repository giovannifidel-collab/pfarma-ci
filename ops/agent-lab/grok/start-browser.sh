...
PROFILE="${GROK_LAB_PROFILE_DIR:-$HOME/.hive-agent-lab/grok-profile}"
PORT="${GROK_LAB_CDP_PORT:-9226}"
DISK_CACHE_BYTES="${GROK_LAB_DISK_CACHE_BYTES:-134217728}"
MEDIA_CACHE_BYTES="${GROK_LAB_MEDIA_CACHE_BYTES:-33554432}"
...
find_browser(){
  if [[ -n "${GROK_LAB_BROWSER_BIN:-}" && -x "${GROK_LAB_BROWSER_BIN}" ]]; then
    printf '%s\n' "$GROK_LAB_BROWSER_BIN"
    return 0
  fi
...
if [[ ! -d node_modules/playwright ]]; then
  echo "GROK LAB: installazione dipendenze Node..."
  npm install
fi

BROWSER="$(find_browser || true)"
if [[ -z "$BROWSER" ]]; then
  echo "GROK LAB: Chromium non presente; bootstrap Playwright Chromium..."
...
for _ in $(seq 1 75); do
  if curl -fsS "http://127.0.0.1:${PORT}/json/version" >/dev/null 2>&1; then
    ...
    echo "Accedi a Grok una sola volta; eventuali verifiche umane vanno completate manualmente."
    exit 0
  fi
  sleep 0.4
done
