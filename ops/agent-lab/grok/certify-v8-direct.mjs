...
const TEST='HIVE-GROK-STRESS-0008-DIRECT';
...
async function createTarget(url){
 return json(`${CDP}/json/new?${encodeURIComponent(url)}`,{method:'PUT'});
}
...
async function focusAndClear(cdp){
 ...
 // Ctrl+A then Backspace using real browser input events.
...
async function send(cdp,text){
 ...
 while(Date.now()-st<15000){
   ...
   if(s.quota)throw new Error('GROK_QUOTA_BLOCKED');
   if(s.text.trim().length<5)return;
   await sleep(250);
 }
...
async function waitResult(cdp,nonce,recheck=false,maxMs=90000){
 const re=resultRegex(nonce,recheck); const start=Date.now();
 while(Date.now()-start<maxMs){
   const body=await bodyText(cdp);
   if(/before limit is gone|Wait or upgrade to SuperGrok for much higher limits/i.test(body))throw new Error('GROK_QUOTA_BLOCKED');
   const m=re.exec(body);
   if(m){await waitReady(cdp);return {sums:{A:+m[1],B:+m[2],C:+m[3]},terms:{A:+m[4],B:+m[5],C:+m[6]},checksum:+m[7]};}
   await sleep(600);
 }
 return null;
}
...
async function trial(index,d){
 const nonce=crypto.randomBytes(4).toString('hex').toUpperCase();
 const exp=expected(d);
 const target=await createTarget('https://grok.com/');
 const cdp=await attach(target);
 try{
   await waitReady(cdp,60000);
   log('TRIAL_START',index,`nonce=${nonce}`,`expected=${exp.checksum}`);
   const ack=`ACK_DATA:${nonce}:${d.salt}:${d.coeff.A},${d.coeff.B},${d.coeff.C}:${d.shards.A.join(',')}|${d.shards.B.join(',')}|${d.shards.C.join(',')}`;
   const dataPrompt=[`${TEST} / ${nonce}`,`SALT=${d.salt}`,`COEFF_A=${d.coeff.A}`,`COEFF_B=${d.coeff.B}`,`COEFF_C=${d.coeff.C}`,`SHARD_A=${d.shards.A.join(',')}`,`SHARD_B=${d.shards.B.join(',')}`,`SHARD_C=${d.shards.C.join(',')}`,`Memorizza questi dati esattamente. Rispondi ESATTAMENTE: ${ack}`].join('\n');
   const before=count(await bodyText(cdp),ack);
   log('SEND DATA');
   await send(cdp,dataPrompt);
   if(!await waitAssistantEcho(cdp,ack,before))throw new Error('TIMEOUT_DATA_ASSISTANT_ECHO');
   log('PASS DATA assistant-echo-verified');

   const resultPrompt=[`${TEST} / ${nonce}`,'Usa SOLO i dati memorizzati nel messaggio DATA. Ricalcola da zero SUM_A, SUM_B, SUM_C; poi TERM_A=COEFF_A*SUM_A, TERM_B=COEFF_B*SUM_B, TERM_C=COEFF_C*SUM_C; infine CHECKSUM=SALT+TERM_A+TERM_B+TERM_C.','Nessuna spiegazione.',`Rispondi ESATTAMENTE: GROK_CERT_RESULT:${nonce}:<SUM_A>,<SUM_B>,<SUM_C>:<TERM_A>,<TERM_B>,<TERM_C>:<CHECKSUM>`].join('\n');
   log('SEND RESULT');
   await send(cdp,resultPrompt);
   const first=await waitResult(cdp,nonce,false);
   if(!first)throw new Error('TIMEOUT_RESULT');
   const firstOk=same(first,exp);
   log('RESULT FIRST',JSON.stringify(first),`OK=${firstOk}`);
   let final=first,rechecked=false;
   if(!firstOk){
     rechecked=true;
     const recheckPrompt=[`${TEST} / ${nonce}`,'Il risultato precedente non supera un controllo interno di coerenza. Non ti fornisco nessun valore atteso. Riparti dai dati originali DATA e ricalcola tutto da zero.','Nessuna spiegazione.',`Rispondi ESATTAMENTE: GROK_RECHECK_RESULT:${nonce}:<SUM_A>,<SUM_B>,<SUM_C>:<TERM_A>,<TERM_B>,<TERM_C>:<CHECKSUM>`].join('\n');
     log('SEND RECHECK');
     await send(cdp,recheckPrompt);
     final=await waitResult(cdp,nonce,true);
     if(!final)throw new Error('TIMEOUT_RECHECK_RESULT');
     log('RESULT RECHECK',JSON.stringify(final),`OK=${same(final,exp)}`);
   }
   const verified=same(final,exp),raw=firstOk;
   log('TRIAL_END',index,`RAW_PASS=${raw}`,`VERIFIED_PASS=${verified}`);
   return {index,nonce,expected:exp,first,final,rechecked,raw_pass:raw,verified_pass:verified};
 } finally {
...
async function main(){
 ...
 // Direct-CDP keeper; no Playwright browser-context attach required.
 const keeper=await createTarget('about:blank');
 console.log('GROK DIRECT-CDP KEEPER READY');
 try{
   const trials=[];
   for(let i=0;i<DATASETS.length;i++)trials.push(await trial(i+1,DATASETS[i]));
