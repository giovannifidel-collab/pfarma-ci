...
const OUT_DIR=process.env.GROK_LAB_CERT_DIR||path.resolve('certifications');
const TEST_ID='HIVE-GROK-STRESS-0001';
...
async function send(page,text){
  ...
  if(!c)throw new Error('GROK_NOT_AUTHENTICATED_OR_COMPOSER_NOT_FOUND');
  await c.click();
  ...
  await c.press('Enter');
  await sleep(700);
  const remaining=await c.innerText().catch(()=>c.inputValue().catch(()=>''));
  if(remaining && remaining.trim().length>10){
    for(const sel of ['button[aria-label*="send" i]','button[data-testid*="send" i]']){
...
async function waitForAssistantExact(page,expected,beforeAssistant,beforeBody,timeout=90000){
  ...
  throw new Error(`TIMEOUT_WAITING_FOR_EXACT_GROK_RESPONSE_${expected}`);
}
...
async function waitFinal(page,timeout=120000){
  const started=Date.now();
  const regex=new RegExp(`GROK_CERT_RESULT:${nonce}:(\\d+):(\\d+),(\\d+),(\\d+):(\\d+),(\\d+),(\\d+):(\\d+),(\\d+),(\\d+):(\\d+)`);
  while(Date.now()-started<timeout){
    const a=await assistantText(page);
    let m=regex.exec(a);
  ...
  throw new Error(`TIMEOUT_WAITING_FOR_GROK_CERT_RESULT_${nonce}`);
}

function parseFinal(m,validation){
  return {
    validation,
    master:{salt:Number(m[1]),coeff:[Number(m[2]),Number(m[3]),Number(m[4])]},
...
async function main(){
  ...
  await send(page,[
    ...
    'Calcola SUM_A, SUM_B, SUM_C; poi TERM_A=COEFF_A*SUM_A, TERM_B=COEFF_B*SUM_B, TERM_C=COEFF_C*SUM_C; infine CHECKSUM=SALT+TERM_A+TERM_B+TERM_C.',
    'Non aggiungere spiegazioni.',
    `Rispondi ESATTAMENTE nel formato: GROK_CERT_RESULT:${nonce}:<SALT>:<COEFF_A>,<COEFF_B>,<COEFF_C>:<SUM_A>,<SUM_B>,<SUM_C>:<TERM_A>,<TERM_B>,<TERM_C>:<CHECKSUM>`
  ].join('\n'));

  const final=await waitFinal(page);
  const passed=
    final.master.salt===MASTER.salt &&
    final.master.coeff.join(',')===MASTER.coeff.join(',') &&
  ...
  const cert={
    test_id:TEST_ID,nonce,provider:'grok.com',transport:'persistent-browser-session',
    api_required:false,zero_cost_api_path:true,started_at:startedAt,completed_at:new Date().toISOString(),
    expected_master:MASTER,actual_master:final.master,expected_sums:SUMS,actual_sums:final.sums,
    expected_terms:TERMS,actual_terms:final.terms,expected_checksum:EXPECTED,actual_checksum:final.checksum,
  ...
  console.log('');
  console.log(`GROK_CERTIFIED=${passed?'true':'false'}`);
  console.log(`TEST_ID=${TEST_ID}`);
  console.log(`NONCE=${nonce}`);
  console.log(`EXPECTED_MASTER=${MASTER.salt}:${MASTER.coeff.join(',')}`);
  console.log(`ACTUAL_MASTER=${final.master.salt}:${final.master.coeff.join(',')}`);
