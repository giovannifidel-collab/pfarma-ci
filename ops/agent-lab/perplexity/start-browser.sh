...
PROFILE="${PERPLEXITY_LAB_PROFILE_DIR:-$HOME/.hive-agent-lab/perplexity-profile}"
PORT="${PERPLEXITY_LAB_CDP_PORT:-9230}"
DISK_CACHE_BYTES="${PERPLEXITY_LAB_DISK_CACHE_BYTES:-134217728}"
MEDIA_CACHE_BYTES="${PERPLEXITY_LAB_MEDIA_CACHE_BYTES:-33554432}"
...
find_browser(){
  if [[ -n "${PERPLEXITY_LAB_BROWSER_BIN:-}" && -x "${PERPLEXITY_LAB_BROWSER_BIN}" ]]; then printf '%s\n' "$PERPLEXITY_LAB_BROWSER_BIN"; return 0; fi
...
if curl -fsS "http://127.0.0.1:${PORT}/json/version" >/dev/null 2>&1; then
  echo "PERPLEXITY LAB BROWSER READY"
...
if [[ ! -d node_modules/playwright ]]; then
  echo "PERPLEXITY LAB: installazione dipendenze Node..."
  npm install
fi

BROWSER="$(find_browser || true)"
if [[ -z "$BROWSER" ]]; then
  echo "PERPLEXITY LAB: Chromium non presente; bootstrap Playwright Chromium..."
...
for _ in $(seq 1 75); do
  if curl -fsS "http://127.0.0.1:${PORT}/json/version" >/dev/null 2>&1; then
    ...
    echo "Accedi a Perplexity manualmente una sola volta se richiesto; non inserire credenziali nel terminale."
    exit 0
  fi
  sleep 0.4
done
