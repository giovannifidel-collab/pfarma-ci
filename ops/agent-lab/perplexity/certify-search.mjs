...
const TEST_ID='HIVE-PERPLEXITY-STRESS-0001';
...
async function waitFinal(timeout=240000){const started=Date.now();const re=new RegExp(`PERPLEXITY_CERT_RESULT:${nonce}:(\\d+):(\\d+):(\\d+):(\\d+):(\\d+):(\\d+):(\\d+):(\\d+):(\\d+):(\\d+):(\\d+)`,'g');while(Date.now()-started<timeout){const b=await bodyText();const matches=[...b.matchAll(re)];if(matches.length)return parseFinal(matches[matches.length-1]);await sleep(1000);}throw new Error(`TIMEOUT_WAITING_FOR_PERPLEXITY_CERT_RESULT_${nonce}`);}
...
async function main(){
  ...
  const cert={test_id:TEST_ID,nonce,provider:'perplexity.ai',product:'Perplexity Web',transport:'browser-session-direct-cdp',mode:{search:mode.searchPressed,computer:mode.computerPressed},login_gate_observed:mode.loginGate,api_required:false,zero_cost_path:true,started_at:startedAt,completed_at:new Date().toISOString(),expected:{master:MASTER,sums:SUMS,terms:TERMS,checksum:EXPECTED},actual:final,stages:{master:true,shard_a:true,shard_b:true,shard_c:true,final:true},stage_validation:'exact-data-echo',certified:passed};
  const file=path.join(OUT_DIR,`${TEST_ID}-${nonce}.json`);fs.writeFileSync(file,JSON.stringify(cert,null,2));
  console.log('');console.log(`PERPLEXITY_CERTIFIED=${passed?'true':'false'}`);console.log(`TEST_ID=${TEST_ID}`);console.log(`NONCE=${nonce}`);console.log(`PRODUCT=Perplexity Web`);console.log(`MODE_SEARCH=${mode.searchPressed}`);console.log(`MODE_COMPUTER=${mode.computerPressed}`);console.log(`LOGIN_GATE_OBSERVED=${mode.loginGate}`);console.log(`EXPECTED_MASTER=${MASTER.salt}:${MASTER.coeff.join(',')}`);console.log(`ACTUAL_MASTER=${final.salt}:${final.coeff.join(',')}`);console.log(`EXPECTED_SUMS=${SUMS.A},${SUMS.B},${SUMS.C}`);console.log(`ACTUAL_SUMS=${final.sums.A},${final.sums.B},${final.sums.C}`);console.log(`EXPECTED_TERMS=${TERMS.A},${TERMS.B},${TERMS.C}`);console.log(`ACTUAL_TERMS=${final.terms.A},${final.terms.B},${final.terms.C}`);console.log(`EXPECTED_CHECKSUM=${EXPECTED}`);console.log(`ACTUAL_CHECKSUM=${final.checksum}`);console.log(`CERTIFICATE=${file}`);
