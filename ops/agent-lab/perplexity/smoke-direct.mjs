...
const expected=`PERPLEXITY_SMOKE_OK:${nonce}`;
...
if(!composer)throw new Error('PERPLEXITY_COMPOSER_NOT_READY');

const before=literalCount(await bodyText(),expected);
const prompt=`HIVE Perplexity smoke test ${nonce}. Rispondi ESATTAMENTE e senza altro testo: ${expected}`;
...
if(!inserted.includes(expected))throw new Error(`PERPLEXITY_TEXT_NOT_INSERTED len=${inserted.length}`);

await call('Input.dispatchKeyEvent',{type:'rawKeyDown',key:'Enter',code:'Enter',windowsVirtualKeyCode:13});
await call('Input.dispatchKeyEvent',{type:'keyUp',key:'Enter',code:'Enter',windowsVirtualKeyCode:13});
console.log('PERPLEXITY_SMOKE_SENT=true');console.log(`NONCE=${nonce}`);

const waitStart=Date.now();let body='';
while(Date.now()-waitStart<150000){
  body=await bodyText();
  if(literalCount(body,expected)>=before+2){console.log('PERPLEXITY_SMOKE_CAPTURE=true');console.log(`EXPECTED=${expected}`);console.log(`ACTUAL=${expected}`);console.log('PERPLEXITY_SMOKE_PASS=true');ws.close();process.exit(0);}
  const low=body.toLowerCase();
  if((low.includes('sign in')||low.includes('log in'))&&(low.includes('continue with google')||low.includes('continue with apple')||low.includes('email'))){console.log('PERPLEXITY_SMOKE_BLOCKED_BY_LOGIN=true');console.log(`BODY_TAIL=${JSON.stringify(body.slice(-6000))}`);ws.close();process.exit(3);}
  await sleep(800);
}
console.log('PERPLEXITY_SMOKE_CAPTURE=false');console.log(`BODY_TAIL=${JSON.stringify(body.slice(-10000))}`);ws.close();process.exit(2);
