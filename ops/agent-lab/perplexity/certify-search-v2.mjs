...
const OUT_DIR=process.env.PERPLEXITY_LAB_CERT_DIR||path.resolve('certifications');
const TEST_ID='HIVE-PERPLEXITY-STRESS-0002';
...
async function composerReady(){return evalJs(`(()=>{const visible=e=>{const r=e.getBoundingClientRect(),s=getComputedStyle(e);return r.width>0&&r.height>0&&s.display!=='none'&&s.visibility!=='hidden'};return [...document.querySelectorAll('[contenteditable="true"],textarea')].some(visible)})()`);}
async function modeInfo(){return evalJs(`(()=>{const visible=e=>{const r=e.getBoundingClientRect(),s=getComputedStyle(e);return r.width>0&&r.height>0&&s.display!=='none'&&s.visibility!=='hidden'};const controls=[...document.querySelectorAll('button,[role="button"]')].filter(visible);const search=controls.find(e=>String(e.innerText||'').trim()==='Search');const computer=controls.find(e=>String(e.getAttribute('aria-label')||'').trim()==='Computer');const dialogs=[...document.querySelectorAll('[role="dialog"],[aria-modal="true"]')].filter(visible);const authInput=[...document.querySelectorAll('input[type="email"],input[name="email"]')].find(visible);const authDialog=dialogs.find(e=>/sign in to continue using perplexity|continue with google|continue with apple|continue with email/i.test(String(e.innerText||'')));return {searchPressed:search?.getAttribute('aria-pressed')??null,computerPressed:computer?.getAttribute('aria-pressed')??null,loginGate:!!authInput||!!authDialog};})()`);}

async function send(text){
  const focused=await evalJs(`(()=>{const visible=e=>{const r=e.getBoundingClientRect(),s=getComputedStyle(e);return r.width>0&&r.height>0&&s.display!=='none'&&s.visibility!=='hidden'};const all=[...document.querySelectorAll('[contenteditable="true"],textarea')].filter(visible);const e=all.find(x=>x.getAttribute('contenteditable')==='true')||all[all.length-1];if(!e)return false;e.focus();return true;})()`);
  if(!focused)throw new Error('PERPLEXITY_COMPOSER_NOT_FOUND');
  ...
  await sleep(400);
  const inserted=await evalJs(`(()=>{const visible=e=>{const r=e.getBoundingClientRect(),s=getComputedStyle(e);return r.width>0&&r.height>0&&s.display!=='none'&&s.visibility!=='hidden'};const all=[...document.querySelectorAll('[contenteditable="true"],textarea')].filter(visible);const e=all.find(x=>x.getAttribute('contenteditable')==='true')||all[all.length-1];return e?String(e.innerText||e.value||''):'';})()`);
  if(!inserted||inserted.length<Math.min(20,text.length))throw new Error('PERPLEXITY_TEXT_NOT_INSERTED');
  await call('Input.dispatchKeyEvent',{type:'rawKeyDown',key:'Enter',code:'Enter',windowsVirtualKeyCode:13});
  await call('Input.dispatchKeyEvent',{type:'keyUp',key:'Enter',code:'Enter',windowsVirtualKeyCode:13});
}

async function visibleAuthGate(){return (await modeInfo()).loginGate;}
async function waitForExact(expected,before,timeout=180000){const started=Date.now();while(Date.now()-started<timeout){const b=await bodyText();if(literalCount(b,expected)>=before+2){await sleep(700);return 'body-occurrence-exact';}if(await visibleAuthGate())throw new Error('PERPLEXITY_LOGIN_BLOCKED_DURING_CERT');await sleep(900);}throw new Error(`TIMEOUT_WAITING_FOR_EXACT_PERPLEXITY_RESPONSE_${expected}`);}
...
function finalMatches(text){const re=new RegExp(`PERPLEXITY_CERT_RESULT:${nonce}:(\\d+):(\\d+):(\\d+):(\\d+):(\\d+):(\\d+):(\\d+):(\\d+):(\\d+):(\\d+):(\\d+)`,'g');return [...text.matchAll(re)];}
async function waitFinal(timeout=240000){
  const started=Date.now();let lastRaw='';let stable=0;
  while(Date.now()-started<timeout){
    const b=await bodyText();const matches=finalMatches(b);
    if(matches.length){
      const m=matches[matches.length-1];const raw=m[0];
      if(raw===lastRaw)stable++;else{lastRaw=raw;stable=1;}
      if(stable>=4){await sleep(1800);const b2=await bodyText();const m2=finalMatches(b2).at(-1);if(m2&&m2[0]===raw)return parseFinal(m2);stable=0;lastRaw=m2?.[0]||'';}
    }
    await sleep(900);
  }
  throw new Error(`TIMEOUT_WAITING_FOR_STABLE_PERPLEXITY_CERT_RESULT_${nonce}`);
}

async function main(){
  fs.mkdirSync(OUT_DIR,{recursive:true});
  const startedAt=new Date().toISOString();
  const start=Date.now();while(Date.now()-start<30000){if(await composerReady())break;await sleep(500);}if(!await composerReady())throw new Error('PERPLEXITY_COMPOSER_NOT_READY');
  const mode=await modeInfo();
  if(mode.loginGate)throw new Error('PERPLEXITY_VISIBLE_LOGIN_GATE');
  log(`START ${TEST_ID} nonce=${nonce} expected=${EXPECTED} sums=${SUMS.A},${SUMS.B},${SUMS.C} terms=${TERMS.A},${TERMS.B},${TERMS.C}`);
  log(`MODE Search=${mode.searchPressed} Computer=${mode.computerPressed} loginGate=${mode.loginGate}`);
  ...
  const cert={test_id:TEST_ID,nonce,provider:'perplexity.ai',product:'Perplexity Web',transport:'browser-session-direct-cdp',mode:{search:mode.searchPressed,computer:mode.computerPressed},login_gate_observed:mode.loginGate,streaming_stability_guard:true,api_required:false,zero_cost_path:true,started_at:startedAt,completed_at:new Date().toISOString(),expected:{master:MASTER,sums:SUMS,terms:TERMS,checksum:EXPECTED},actual:final,stages:{master:true,shard_a:true,shard_b:true,shard_c:true,final:true},stage_validation:'exact-data-echo',certified:passed};
  const file=path.join(OUT_DIR,`${TEST_ID}-${nonce}.json`);fs.writeFileSync(file,JSON.stringify(cert,null,2));
  console.log('');console.log(`PERPLEXITY_CERTIFIED=${passed?'true':'false'}`);console.log(`TEST_ID=${TEST_ID}`);console.log(`NONCE=${nonce}`);console.log(`PRODUCT=Perplexity Web`);console.log(`MODE_SEARCH=${mode.searchPressed}`);console.log(`MODE_COMPUTER=${mode.computerPressed}`);console.log(`LOGIN_GATE_OBSERVED=${mode.loginGate}`);console.log('STREAMING_STABILITY_GUARD=true');console.log(`EXPECTED_MASTER=${MASTER.salt}:${MASTER.coeff.join(',')}`);console.log(`ACTUAL_MASTER=${final.salt}:${final.coeff.join(',')}`);console.log(`EXPECTED_SUMS=${SUMS.A},${SUMS.B},${SUMS.C}`);console.log(`ACTUAL_SUMS=${final.sums.A},${final.sums.B},${final.sums.C}`);console.log(`EXPECTED_TERMS=${TERMS.A},${TERMS.B},${TERMS.C}`);console.log(`ACTUAL_TERMS=${final.terms.A},${final.terms.B},${final.terms.C}`);console.log(`EXPECTED_CHECKSUM=${EXPECTED}`);console.log(`ACTUAL_CHECKSUM=${final.checksum}`);console.log(`CERTIFICATE=${file}`);
  ws.close();if(!passed)process.exit(2);
}
main().catch(err=>{console.error('PERPLEXITY_CERTIFIED=false');console.error(`ERROR=${err.message}`);try{ws.close();}catch{}process.exit(1);});
