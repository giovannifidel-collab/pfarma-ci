...
const TEST_ID='HIVE-COPILOT-STRESS-0003';
...
async function pressEnter(){await call('Input.dispatchKeyEvent',{type:'rawKeyDown',key:'Enter',code:'Enter',windowsVirtualKeyCode:13});await call('Input.dispatchKeyEvent',{type:'keyUp',key:'Enter',code:'Enter',windowsVirtualKeyCode:13});return true;}
async function verifySubmitted(stageToken,timeout=8000){const start=Date.now();while(Date.now()-start<timeout){const s=await uiState();const b=await bodyText();if(!s.composerText.includes(nonce)&&literalCount(b,stageToken)>=1)return true;await sleep(250);}return false;}

async function send(text,stageToken,label){
  if(!await waitIdleStable())throw new Error(`COPILOT_NOT_IDLE_BEFORE_${label}`);
  await clearComposer();
  ...
  for(const [name,fn] of attempts){
    if(await verifySubmitted(stageToken,700)){log('SUBMIT_ALREADY_CONFIRMED',label,name);return `confirmed-before-${name}`;}
    if(!await waitIdleStable(15000)){
      s=await uiState();
      if(!s.composerText.includes(nonce)&&literalCount(await bodyText(),stageToken)>=1)return `submitted-while-idle-transition`;
    }
    s=await uiState();
    if(!s.composerText.includes(nonce)&&literalCount(await bodyText(),stageToken)>=1)return `submitted-before-${name}`;
    if(!s.composerText.includes(nonce))throw new Error(`COPILOT_COMPOSER_LOST_PROMPT_${label}_${name}`);
    log('SUBMIT_ATTEMPT',label,name);
    await fn();
    if(await verifySubmitted(stageToken,9000))return name;
  }
  throw new Error(`COPILOT_PROMPT_NOT_SUBMITTED_AFTER_RETRIES_${stageToken}`);
}

async function waitForExact(expected,before,timeout=180000){const started=Date.now();while(Date.now()-started<timeout){const b=await bodyText();if(literalCount(b,expected)>=before+2){await sleep(1600);return 'body-occurrence-exact';}await sleep(750);}throw new Error(`TIMEOUT_WAITING_FOR_EXACT_COPILOT_RESPONSE_${expected}`);}
...
async function main(){
  ...
  const cert={test_id:TEST_ID,nonce,provider:'Microsoft Copilot',product:'Microsoft Copilot Web',transport:'browser-session-direct-cdp',mode:'Standard chat',authenticated:state.account,think_deeper_discovered:false,streaming_stability_guard:true,send_handshake:'idle-stable+dom-click/cdp-mouse/enter+composer-cleared+prompt-visible',api_required:false,zero_cost_path:true,started_at:startedAt,completed_at:new Date().toISOString(),expected:{master:MASTER,sums:SUMS,terms:TERMS,checksum:EXPECTED},actual:final,stages:{master:true,shard_a:true,shard_b:true,shard_c:true,final:true},stage_validation:'exact-data-echo',certified:passed};
  const file=path.join(OUT_DIR,`${TEST_ID}-${nonce}.json`);fs.writeFileSync(file,JSON.stringify(cert,null,2));
  console.log('');console.log(`COPILOT_CERTIFIED=${passed?'true':'false'}`);console.log(`TEST_ID=${TEST_ID}`);console.log(`NONCE=${nonce}`);console.log('PRODUCT=Microsoft Copilot Web');console.log('MODE=Standard chat');console.log(`AUTHENTICATED=${state.account?'true':'false'}`);console.log('THINK_DEEPER_DISCOVERED=false');console.log('STREAMING_STABILITY_GUARD=true');console.log('SEND_HANDSHAKE=idle-stable+multi-submit-fallback+composer-cleared+prompt-visible');console.log(`EXPECTED_MASTER=${MASTER.salt}:${MASTER.coeff.join(',')}`);console.log(`ACTUAL_MASTER=${final.salt}:${final.coeff.join(',')}`);console.log(`EXPECTED_SUMS=${SUMS.A},${SUMS.B},${SUMS.C}`);console.log(`ACTUAL_SUMS=${final.sums.A},${final.sums.B},${final.sums.C}`);console.log(`EXPECTED_TERMS=${TERMS.A},${TERMS.B},${TERMS.C}`);console.log(`ACTUAL_TERMS=${final.terms.A},${final.terms.B},${final.terms.C}`);console.log(`EXPECTED_CHECKSUM=${EXPECTED}`);console.log(`ACTUAL_CHECKSUM=${final.checksum}`);console.log(`CERTIFICATE=${file}`);
