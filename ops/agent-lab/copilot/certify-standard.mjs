...
const TEST_ID='HIVE-COPILOT-STRESS-0001';
...
async function uiState(){return evalJs(`(()=>{const visible=e=>{const r=e.getBoundingClientRect(),s=getComputedStyle(e);return r.width>0&&r.height>0&&s.display!=='none'&&s.visibility!=='hidden'};const all=[...document.querySelectorAll('[contenteditable="true"],textarea')].filter(visible);const composer=all.find(x=>/message copilot/i.test(x.getAttribute('aria-label')||x.getAttribute('placeholder')||''));const account=[...document.querySelectorAll('button,[role="button"],a')].filter(visible).find(e=>/personal account/i.test(e.getAttribute('aria-label')||''));const visibleText=[...document.querySelectorAll('main,section,div')].filter(visible).map(e=>String(e.innerText||'').trim()).filter(Boolean);const signIn=visibleText.some(t=>/^sign in to copilot$/i.test(t)||/^sign in$/i.test(t)&&t.length<20);return {composer:!!composer,account:!!account,signIn};})()`);}
async function composerReady(){return (await uiState()).composer;}

async function send(text){
  const focused=await evalJs(`(()=>{const visible=e=>{const r=e.getBoundingClientRect(),s=getComputedStyle(e);return r.width>0&&r.height>0&&s.display!=='none'&&s.visibility!=='hidden'};const all=[...document.querySelectorAll('[contenteditable="true"],textarea')].filter(visible);const e=all.find(x=>/message copilot/i.test(x.getAttribute('aria-label')||x.getAttribute('placeholder')||''));if(!e)return false;e.focus();return true;})()`);
  ...
  if(!inserted||!inserted.includes(nonce))throw new Error('COPILOT_TEXT_NOT_INSERTED');
  await call('Input.dispatchKeyEvent',{type:'rawKeyDown',key:'Enter',code:'Enter',windowsVirtualKeyCode:13});
  await call('Input.dispatchKeyEvent',{type:'keyUp',key:'Enter',code:'Enter',windowsVirtualKeyCode:13});
}

async function waitForExact(expected,before,timeout=180000){const started=Date.now();while(Date.now()-started<timeout){const b=await bodyText();if(literalCount(b,expected)>=before+2){await sleep(900);return 'body-occurrence-exact';}const s=await uiState();if(!s.composer&&s.signIn)throw new Error('COPILOT_LOGIN_BLOCKED_DURING_CERT');await sleep(900);}throw new Error(`TIMEOUT_WAITING_FOR_EXACT_COPILOT_RESPONSE_${expected}`);}
