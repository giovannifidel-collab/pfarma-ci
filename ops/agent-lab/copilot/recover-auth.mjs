...
let target=targets.find(t=>t.type==='page'&&/auth\.copilot\.microsoft\.com\/login\/callback/i.test(t.url||''))||targets.find(t=>t.type==='page'&&/(?:copilot\.microsoft\.com|copilot\.com)/i.test(t.url||''));
...
while(Date.now()-start<45000){
  await sleep(700);
  state=await evalJs(`(()=>{const visible=e=>{const r=e.getBoundingClientRect(),s=getComputedStyle(e);return r.width>0&&r.height>0&&s.display!=='none'&&s.visibility!=='hidden'};const controls=[...document.querySelectorAll('textarea,[contenteditable="true"],input')].filter(visible);const actions=[...document.querySelectorAll('a,button,[role="button"]')].filter(visible).map(e=>({text:String(e.innerText||'').trim(),aria:String(e.getAttribute('aria-label')||'').trim(),href:e.href||null}));const signIn=actions.some(x=>/sign in/i.test(x.text)||/sign in/i.test(x.aria));const chat=controls.some(e=>{const p=String(e.getAttribute('placeholder')||'').toLowerCase(),a=String(e.getAttribute('aria-label')||'').toLowerCase();return p.includes('message copilot')||a.includes('message copilot')});return {href:String(location.href||''),title:String(document.title||''),ready:document.readyState,visibleControls:controls.length,signIn,chat};})()`).catch(()=>({}));
  if(state.ready==='complete'&&/(?:copilot\.microsoft\.com|copilot\.com)/i.test(state.href||''))break;
}
console.log('=== COPILOT AUTH RECOVERY ===');
console.log(`URL=${state.href||''}`);
console.log(`READY_STATE=${state.ready||''}`);
console.log(`VISIBLE_CONTROLS=${state.visibleControls??0}`);
console.log(`SIGNIN_VISIBLE=${state.signIn?'true':'false'}`);
console.log(`CHAT_READY=${state.chat?'true':'false'}`);
