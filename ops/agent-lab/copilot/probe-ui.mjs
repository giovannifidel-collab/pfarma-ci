...
if(!target){target=await json(`${CDP}/json/new?${encodeURIComponent('https://copilot.microsoft.com/')}`,{method:'PUT'});await sleep(2500);}
...
const info=await evalJs(`(()=>{
  ...
  const buttons=[...document.querySelectorAll('button,[role="button"],a')].filter(visible).map(e=>({text:String(e.innerText||'').trim(),aria:e.getAttribute('aria-label'),title:e.getAttribute('title'),href:e.getAttribute('href'),pressed:e.getAttribute('aria-pressed')})).filter(x=>x.text||x.aria||x.title).slice(-120);
  const dialogs=[...document.querySelectorAll('[role="dialog"],[aria-modal="true"]')].filter(visible).map(e=>String(e.innerText||'').slice(0,1500));
  const login=dialogs.some(t=>/sign in|log in|microsoft account|continue/i.test(t))||location.href.includes('login.live.com')||location.href.includes('login.microsoftonline.com');
  const chat=controls.some(x=>x.tag==='textarea'||x.contenteditable==='true')&&(low.includes('copilot')||low.includes('message')||low.includes('ask')||low.includes('what can i help'));
  return {href:location.href,title:document.title,ready:document.readyState,login,chat,controls,buttons,dialogs,tail:body.slice(-12000)};
})()`);
console.log('=== COPILOT UI PROBE ===');
console.log(`URL=${info.href}`);console.log(`TITLE=${info.title}`);console.log(`READY_STATE=${info.ready}`);console.log(`LOGIN_HINT=${info.login}`);console.log(`CHAT_HINT=${info.chat}`);console.log(`CONTROLS=${JSON.stringify(info.controls,null,2)}`);console.log(`BUTTONS=${JSON.stringify(info.buttons,null,2)}`);console.log(`DIALOGS=${JSON.stringify(info.dialogs,null,2)}`);console.log(`BODY_TAIL=${JSON.stringify(info.tail)}`);
ws.close();
