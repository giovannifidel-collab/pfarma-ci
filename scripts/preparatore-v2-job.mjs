...
const catalog = {
  incline_pushup: ["Push-up inclinati", "Spinta", "reps", []], floor_pushup: ["Push-up a terra", "Spinta", "reps", []], pike_pushup: ["Pike push-up", "Spinta", "reps", []], close_grip_pushup: ["Push-up presa stretta", "Spinta", "reps", []], shoulder_tap: ["Shoulder tap controllato", "Spinta", "reps", []],
  band_lateral_raise: ["Alzate laterali con elastico sotto i piedi", "Spinta", "reps", ["elastici"]], dumbbell_lateral_raise: ["Alzate laterali con manubri", "Spinta", "reps", ["manubri"]], band_overhead_triceps_extension: ["Estensione tricipiti sopra la testa con elastico sotto i piedi", "Spinta", "reps", ["elastici"]], dumbbell_overhead_triceps_extension: ["Estensione tricipiti sopra la testa con manubrio", "Spinta", "reps", ["manubri"]], cable_triceps_pushdown: ["Pushdown tricipiti ai cavi", "Spinta", "reps", ["cavi"]],
...
const livePhases = new Set(["runner_claimed", "codex_analysis", "plus_audit", "repair_plan", "final_validation", "publishing"]);
...
async function fetchJob() { const response = await fetch(restUrl(`preparatore_v2_jobs?id=eq.${jobId}&select=*`), { headers: headers(), cache: "no-store" }); if (!response.ok) throw new Error(`Job lookup failed (${response.status})`); const rows = await response.json(); const job = rows[0]; if (!job || job.user_id !== expectedUserId) throw new Error("Job not found or owner mismatch"); return job; }
...
async function prepare() {
  ...
  const claimed = await patchJob(`id=eq.${jobId}&user_id=eq.${expectedUserId}&status=eq.queued`, { status: "running", error_code: "runner_claimed", error_message: null, started_at: new Date().toISOString(), attempt_count: Number(job.attempt_count || 0) + 1 }, "return=representation");
...
function hasExplicitArmsPriority(profile, analysis) { const profileText = JSON.stringify({ goal: profile?.goal, priorities: profile?.priorities, preference: profile?.preference }).toLowerCase(); return /bracci|bicip|tricip/.test(profileText) || analysis?.priorities?.some((priority) => priority?.area === "Braccia" || priority?.area === "Proporzione braccia / tronco"); }
...
function normalizeTimeBudget(day, warmupSeconds) {
  ...
  if (budget.total !== day.durationMinutes || budget.warmup !== day.warmupMinutes) throw new Error("Time budget mismatch");
  const estimatedWorkSeconds = estimateExerciseBlockSeconds(day.exercises);
  const workMinutes = Math.ceil(estimatedWorkSeconds / 60);
  const minTransitionMinutes = Math.ceil(minimumTransitionSeconds(day.exercises.length) / 60);
  const availableMinutes = day.durationMinutes - day.warmupMinutes - budget.finisher;
  const transitionMinutes = availableMinutes - workMinutes;
  if (transitionMinutes < minTransitionMinutes || transitionMinutes > 10 || workMinutes < 5 || workMinutes > 50) throw new Error(`Time audit failed on day ${day.day}: content does not fit duration`);
  const estimatedTotalSeconds = warmupSeconds + estimatedWorkSeconds + minimumTransitionSeconds(day.exercises.length) + budget.finisher * 60;
  ...
  return { warmup: day.warmupMinutes, work: workMinutes, transitions: transitionMinutes, finisher: budget.finisher, total: day.durationMinutes };
}
...
function normalizeResult(raw, job) {
  ...
  for (const priority of analysis.priorities) if (!priority || !allowedAreas.has(priority.area) || !["low", "medium"].includes(priority.confidence) || !safeText(priority.observation, 500) || !safeText(priority.trainingImplication, 500)) throw new Error("Invalid strategic priority");
  const safetyText = safetyScanText(raw);
  if (/percentuale\s*(?:di\s*)?grasso[^0-9]{0,20}\d/i.test(safetyText) || /(?:presenta|mostra|compatibile con|segni di|evidenza di)\s+(?:una\s+)?(?:scoliosi|iperlordosi|ginecomastia)/i.test(safetyText)) throw new Error("Unsupported photo inference");
  ...
  const days = raw.plan.days.map((day, index) => {
    ...
    const warmupSteps = day.warmupSteps.map((step) => { if (!safeText(step?.name, 100) || !integer(step?.seconds, 20, 180) || !safeText(step?.instruction, 240)) throw new Error("Invalid warmup step"); return { name: step.name, seconds: step.seconds, instruction: step.instruction }; });
    ...
    for (const exercise of day.exercises) {
      ...
      if (mode === "time") { if (!integer(exercise.seconds, 10, 180) || exercise.repsMin !== null || exercise.repsMax !== null) throw new Error("Invalid time target"); }
      else if (!integer(exercise.repsMin, 1, 30) || !integer(exercise.repsMax, exercise.repsMin, 40) || exercise.seconds !== null) throw new Error("Invalid rep target");
    ...
    const budget = normalizeTimeBudget(day, warmupSeconds);
    const exercises = day.exercises.map((exercise) => {
      const [name, focus, mode, requires] = catalog[exercise.exerciseId];
      if (mode === "time") return { name, prescription: `${exercise.sets} x ${exercise.seconds} s`, restSeconds: exercise.restSeconds, focus, target: { mode: "time", sets: exercise.sets, secondsMin: exercise.seconds, secondsMax: exercise.seconds }, techniqueCue: exercise.techniqueCue, ...(requires.length ? { requires } : {}) };
...
async function fail() { const job = await fetchJob(); const code = process.argv[5] === "expired" ? "expired" : "codex_failed"; await patchJob(`id=eq.${jobId}&user_id=eq.${expectedUserId}&status=in.(queued,running)`, { status: "failed", error_code: code, error_message: "Analisi Codex non completata. Nessuna scheda è stata modificata.", completed_at: new Date().toISOString() }); await deletePhotos(job.storage_paths); }

if (command === "prepare") await prepare();
else if (command === "phase") await setPhase(process.argv[5]);
else if (command === "complete") await complete();
else if (command === "fail") await fail();
else throw new Error("Usage: preparatore-v2-job.mjs prepare|phase|complete|fail JOB_ID JOB_DIR [phase]");
