...
def main():
    ...
    try:
        ...
        wait(session,"document.querySelector('#detail-dialog')?.open===true"); wait(session,"document.querySelector('.consumer-detail-layout')"); wait(session,"document.querySelector('[data-durable-tags] .consumer-tag-edit')")
        detail_contract=execute(session,"""const title=document.querySelector('.consumer-detail-title')?.textContent?.trim();const album=document.querySelector('[data-detail-album-status]')?.textContent?.trim()||'';const tech=document.querySelector('.consumer-technical-details');const editor=document.querySelector('.consumer-tag-edit');return {title,album,technicalClosed:Boolean(tech&&!tech.open),tagEditorClosed:Boolean(editor&&!editor.open),filenameInTechnical:/Nome file/.test(tech?.textContent||''),membershipJargon:/membership/i.test(document.querySelector('.consumer-detail-sidebar')?.textContent||'')};""")
        if detail_contract['title'] not in ['Foto','Video'] or detail_contract['membershipJargon'] or not detail_contract['technicalClosed'] or not detail_contract['tagEditorClosed'] or not detail_contract['filenameInTechnical']: raise RuntimeError(f'detail contract failed {detail_contract}')
        desktop['detail']=detail_contract; shot(session,'desktop-detail.png'); execute(session,"document.querySelector('#close-detail')?.click();return true;")
