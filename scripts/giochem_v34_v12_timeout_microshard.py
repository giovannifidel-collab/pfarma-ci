...
EXECUTION_MODE = 'V12_DETERMINISTIC_MICROSHARD_PER_DOCKING_TIMEOUT_ONLY_RECOVERY'
...
def install_timeout_only_patch(worker, timeout_seconds: int) -> None:
    ...
    # Frozen score_receptor imports the stdlib subprocess module and calls
    # subprocess.run(..., timeout=900). Intercept exactly that technical timeout
    # and leave argv, cwd, environment, capture/check semantics and every
    # scientific docking parameter untouched.
    real_run = subprocess.run
    frozen_run = worker.dev.subprocess.run
    if frozen_run is not real_run:
        raise SystemExit('FAIL-CLOSED unexpected subprocess hook before V12 patch')
...
def score_micro(args) -> None:
    ...
    policy, runtime, exe, receptors, receptor_sha, protocol = validate_frozen(
        worker, args.target, args.runtime_lock_sha256, Path(args.receptor_dir), rmanifest
    )
    ...
    meta = {
        'recordType':'metadata', 'suite':policy['suite'], 'version':'3.4.0-external-blind1',
        'target':args.target, 'scientificRole':'BLIND_HOLDOUT_SCORING_NO_LABELS',
        'ranker':'rank_consensus_all3', 'requiredSignals':['cnnaffinity_max','vina_best','cnnscore_mean3'],
        'shardIndex':args.logical_shard, 'shardCount':args.logical_count,
        ...
        'gninaBinarySha256':runtime['gninaSha256'], 'sourceCodeSha':args.source_code_sha,
        'truthAvailableToWorker':False, 'labelsRead':False, 'numModes':policy['docking']['numModes'],
        'receptorWeights':[0.5,0.5], 'executionMode':EXECUTION_MODE,
    ...
    with tempfile.TemporaryDirectory(prefix=f'giochem-v34-v12-{args.target}-{args.logical_shard}-{args.micro_index}-') as td, out.open('w', encoding='utf-8') as fh:
        ...
        for n, row in enumerate(selected, 1):
            ...
            fh.flush()
            if n % 2 == 0:
                print(f'V12 {args.target} logical {args.logical_shard} micro {args.micro_index}: {n}/{len(selected)}', flush=True)
...
def recombine_one(args) -> None:
    ...
    for path in files:
        ...
        if meta.get('executionMode') != EXECUTION_MODE or int(meta.get('executionMicroCount', -1)) != args.micro_count or mi in seen_micro:
            raise SystemExit('FAIL-CLOSED micro execution provenance drift')
        tr = meta.get('technicalRecovery') or {}
        if tr.get('kind') != 'perDockingTimeoutOnly' or int(tr.get('frozenSeconds', -1)) != FROZEN_TIMEOUT_SECONDS:
            raise SystemExit('FAIL-CLOSED timeout-only provenance drift')
        seen_micro.add(mi)
...
def parser() -> argparse.ArgumentParser:
    ...
    s.add_argument('--micro-index', type=int, required=True); s.add_argument('--micro-count', type=int, required=True); s.add_argument('--gnina-timeout-seconds', type=int, required=True)
    s.add_argument('--require-zero-failures', action='store_true'); s.add_argument('--output', required=True)
...
def main() -> None:
    ...
    if args.command == 'score': score_micro(args)
    else: recombine_one(args)
